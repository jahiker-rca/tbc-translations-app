...
// Configuration
const ADMIN_API_URL = `https://${process.env.SHOPIFY_SHOP_NAME}/admin/api/${process.env.SHOPIFY_API_VERSION}`;
...
async function createTranslation(productId, namespace, key, translatedValue, targetLocale) {
    try {
...
async function getOrCreateMetafieldTranslation(productId, namespace, key, targetLocale = 'de') {
    try {
        ...
        const mutationResult = await createTranslation(
